...
const DigitalConsent = () => (
  <blockquote className="legal-consent">
    <strong>Texto que debe aparecer junto a la aceptación de condiciones en el checkout:</strong>
    <br />
...
function CondicionesCompra() {
  return (
    <LegalShell title="Condiciones de compra" intro="Estas condiciones explican con claridad qué compras, cuánto pagas, cómo se entrega y qué ocurre con el desistimiento del contenido digital.">
      ...
      <section><h2>3. Precio e impuestos</h2><p>El precio de cada guía individual es {legalConfig.prices.individual} y el precio del pack es {legalConfig.prices.bundle}. {legalConfig.prices.taxNotice}</p><p>No se añaden gastos de envío porque la entrega es digital. El precio aplicable será el que se muestre en el checkout de Payhip inmediatamente antes de pulsar el botón de pago.</p></section>
      <section><h2>4. Cómo se realiza la compra</h2><p>La persona compradora selecciona el producto en Matavicios y será dirigida al checkout de Payhip para completar el pago. Antes de pagar podrá revisar el producto, el precio total, los impuestos aplicables y las presentes condiciones. El contrato se perfecciona cuando el pago es autorizado y el vendedor confirma la compra por correo electrónico.</p><p>Los pagos se procesan por {legalConfig.delivery.paymentProvider}. El vendedor no almacena datos completos de tarjetas bancarias.</p></section>
...
function Cookies() {
  return (
    <LegalShell title="Política de cookies" intro="Esta página explica qué tecnologías de almacenamiento usa Matavicios y cuándo se pedirá tu consentimiento.">
      ...
      <section><h2>2. Configuración actual del sitio</h2><p>En su configuración actual, Matavicios no utiliza cookies de publicidad, perfiles comerciales ni analítica opcional en esta web. Solo se utilizarán, si fueran necesarias, tecnologías técnicas o de seguridad imprescindibles para mostrar la página, mantener su funcionamiento o prevenir abusos. Estas tecnologías no requieren consentimiento cuando sean estrictamente necesarias.</p><p>El pago y la entrega se realizan en el sitio de Payhip, que es un servicio distinto y puede usar Stripe para procesar el pago. Cada proveedor cuenta con sus propias políticas de privacidad y cookies; la persona usuaria podrá consultarlas dentro del entorno de pago antes de completar la compra.</p></section>
